...
public class HttpDecoder implements TcpProcessor {
	...
	public void handleTx(TcpSessionKey sessionKey, Buffer data) {
		...
		handleRequest(session, data, sessionKey.getClientIp(), sessionKey.getServerIp() );
	}

	@Override
	public void handleRx(TcpSessionKey sessionKey, Buffer data) {
		HttpSession session = sessionMap.get(sessionKey);
		handleResponse(session, data, sessionKey.getClientIp(), sessionKey.getServerIp());
	}
	...
	public void onFinish(TcpSessionKey session) {
		...
		handleNoContentLengthCase(httpSession, session.getClientIp(), session.getServerIp());
	...
	private void handleNoContentLengthCase(HttpSession httpSession, InetAddress ClientIp, InetAddress ServerIp) {
		if (httpSession != null && httpSession.getResponseState() == HttpResponseState.GOT_HEADER) {
			decodeContent(httpSession.getResponse());
			dispatchResponse(httpSession, ClientIp, ServerIp);
		}
	}

	private void handleRequest(HttpSession session, Buffer data,InetAddress ClientIp, InetAddress ServerIp) {
		Buffer txBuffer = session.getTxBuffer();
		txBuffer.addLast(data);
		parseRequest(session, txBuffer, ClientIp, ServerIp);
	}

	private void handleResponse(HttpSession session, Buffer data,InetAddress ClientIp, InetAddress ServerIp) {
		int capacity = data.readableBytes();
		Buffer rxBuffer = session.getRxBuffer();
		rxBuffer.addLast(data);
		parseResponse(session, rxBuffer, data, capacity, ClientIp, ServerIp);
	}

	private void parseRequest(HttpSession session, Buffer txBuffer, InetAddress ClientIp, InetAddress ServerIp) {
		...
		while (session.getRequestState() != HttpRequestState.END) {
			...
			case GOT_HEADER:
				...
				dispatchRequest(request, ClientIp, ServerIp);
	...
	private void parseResponse(HttpSession session, Buffer rxBuffer, Buffer data, int capacity, InetAddress ClientIp, InetAddress ServerIp) {
		...
		while (session.getResponseState() != HttpResponseState.END) {
			...
			case GOT_HEADER:
				...
				dispatchResponse(session, ClientIp, ServerIp);
	...
	private void dispatchRequest(HttpRequestImpl request, InetAddress ClientIp, InetAddress ServerIp) {
		for (HttpProcessor processor : callbacks) {
			processor.onRequest(request,ClientIp, ServerIp);
		}
	}

	private void dispatchResponse(HttpSession session, InetAddress ClientIp, InetAddress ServerIp) {
		session.getResponse().setContent();

		for (HttpProcessor processor : callbacks) {
			processor.onResponse( session.getRequest(), session.getResponse(), ClientIp, ServerIp);
		}
	}
